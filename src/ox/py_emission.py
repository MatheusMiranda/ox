...
class Options:
    ...
    def variable(self, _type, name):
        ctx.tokens,append(name)

    def type(self, _type):
        ctx.tokens.append(_type)
...
expr10 = SimpleStatement(IfBlock(Equal(Name('n'), Number('1')),block2))
expr4 = ForBlock(Name('x'), Number(2), block2)
block3 = Block([expr10, expr4])

expr3 = WhileBlock(OrOp(AndOp(Equal(Name('x'), Boolean('true')), Equal(Name('y'), Number(10))),\
    AndOp(Equal(Name('x'), Number(2)), Equal(Name('y'), Number(10)))), block3)


block = Block([expr3, returnf])

test1 = Arg('int', 't1')
test2 = Arg('int', 't2')

expr = FunDef(Type(''),Name('teste'),FunArg([test1,test2]),block)
print(source(expr))
